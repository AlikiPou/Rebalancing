...
for i in datafiles:
    ...
    '''
    # status at station, before start of redistribution
    status_node = {i: rnd.randint(0, 6) for i in nodes}
    status_node[0] = 0
    print("The initial status in node (i) is:", status_node)
    '''
    ...
    '''
    # Forecasted demand in node i
    forecast_node = {i: rnd.randint(1, 6) for i in nodes}
    forecast_node[0] = 0
    '''

    # print("The forecasted demand in node (i) is:", forecast_node)
    # Vehicle capacity
    # q = 20

    '''
    # parking station capacity
    P = {i: rnd.randint(6, 8) for i in nodes}
    P[0] = 0
    print("The parking capacity in node (i) is:", P)
    '''
    ...
    l = {(i, k): Rebalancing.addVar(vtype=grb.GRB.INTEGER, lb=-10000000,
    ...
    y_l = {(i, k): Rebalancing.addVar(vtype=grb.GRB.INTEGER,
                                      name=f"y_l_{i}_{k}".format(i, k))
           for i in nodes for k in vehicles}

    # Number of bikes unloaded on vehicle k in station i
    y_u = {(i, k): Rebalancing.addVar(vtype=grb.GRB.INTEGER,
        ...
        for h in validCut1_subset:
            for i in locations:
                for j in locations:
                    if (i, j) == h:
                        Rebalancing.addConstr(
                            x[i, j, k] + grb.quicksum(x[j, h, k] for h in validCut1_subset[i, j]) <= 1)
